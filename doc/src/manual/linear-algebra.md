...
of tranpose operations in matrix-matrix solutions. Many of these are further specialized for certain special
matrix types. For example, `A\B` will end up calling [`A_ldiv_B`](@ref) while `A'\B` will end up calling
[`Ac_ldiv_B`](@ref), even though we used the same left-division operator. This works for matrices too: `A.'\B.'`
would call [`At_ldiv_Bt`](@ref). The left-division operator is pretty powerful and it's easy to write compact,
